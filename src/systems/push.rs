use amethyst::{
    core::{Named, transform::Transform},
    derive::SystemDesc,
    ecs::prelude::{Entities, Join, ReadStorage, System, SystemData, WriteStorage},
};

use crate::components::{Escalator, Step, Thing, Push};
...
impl<'s> System<'s> for PushSystem {
    ...
    fn run(&mut self, (
        ...
        for (thing_entity, thing_push, thing, thing_transform) in (&entities, &mut atops, &things, &transforms).join() {
...
fn calculate_pushness(thing: &Thing, thing_transform: &Transform, step: &Step, step_transform: &Transform) -> f32 {
    ...
    let overlap =  f32::min(step_bounds.top() - thing_bounds.bottom(), thing_bounds.top() - step_bounds.bottom());
